...
class SlashCommandParserTestCase(unittest.TestCase):
    ...
    def test_relative_time_parser(self):
        initial_time = datetime.datetime(2019, 8, 19, 0, 17, 5)
        
        # 1 hour
        time_elapsed = datetime.timedelta(hours=1)
        ...
        # 2 hours
        time_elapsed = datetime.timedelta(hours=2)
        ...
        # 1 hour, 30 minutes
        time_elapsed = datetime.timedelta(hours=1, minutes=30)
        ...
        # 1 hour, 15 minutes
        time_elapsed = datetime.timedelta(hours=1, minutes=15)
        p = SlashCommandParser("1hr 15min say Take a break", initial_time)
        self.assertEqual(p.get_time(), initial_time + time_elapsed)
        
        # 1 day
        time_elapsed = datetime.timedelta(day=1)
        ...
        # 2 days
        time_elapsed = datetime.timedelta(days=2)
        p = SlashCommandParser("2 days say Take a break", initial_time)
        self.assertEqual(p.get_time(), initial_time + time_elapsed)
        
        # 1 day, 50 minutes
        time_elapsed = datetime.timedelta(days=1, minutes=50)
    ...
    def test_relatively_absolute_time_parser(self):
        ...
        # Next Monday
        final_datetime = datetime.datetime(2019, 8, 26, 0, 17, 5)
        ...
        p = SlashCommandParser(
            "8 a.m. say Take a break", datetime.datetime(2019, 8, 19, 8, 17, 5))
        self.assertEqual(p.get_time(), final_datetime)
        ...
        final_datetime = datetime.datetime(2019, 8, 19, 12, 17, 5)
    ...
    def test_absolute_time_parser(self):
        ...
        p = SlashCommandParser("september 1 say Take a break")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("sep 1 say Blah")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("sep 1, 2019 say Hi, there!")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("sep 1 2019 say Hi, there!")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("1 sep say Take a break")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("2019-09-01 say Blah")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("09-01 say Hi, there!")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("9/1 say Take a break")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("2019/09/01 say Blah")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("09/01/2019 say Hi, there!")
        self.assertEqual(p.get_time(), final_datetime)
        
        # September 1, 2019, 8:00am
        final_datetime = datetime.datetime(2019, 9, 1, 8, 0, 0)
        p = SlashCommandParser("8a sep 1 say Take a break")
        self.assertEqual(p.get_time(), final_datetime)
        
        # September 1, 2019, 2:30pm GMT
        final_datetime = datetime.datetime(2019, 9, 1, 14, 30, 0)
        p = SlashCommandParser("2019-09-01 14:30 say Take break")
        self.assertEqual(p.get_time(), final_datetime)
        
        p = SlashCommandParser("2019-09-01T14:30:30Z say Blah")
        self.assertEqual(p.get_time(), final_datetime)
    
    def test_time_string(self):
        initial_time = datetime.datetime(2019, 8, 19, 0, 17, 59)
        
        p = SlashCommandParser("1 hour say meh", initial_time)
        self.assertEqual(p.get_time_string().lower(), "2019-08-19 at 01:18 am")
        
        p = SlashCommandParser("3 min say meh", initial_time)
        self.assertEqual(p.get_time_string().lower(), "2019-08-19 at 00:31 am")
        
        p = SlashCommandParser("30 seconds say meh", initial_time)
        self.assertEqual(p.get_time_string().lower(), "2019-08-19 at 00:18 am")
        
        p = SlashCommandParser("2 days 50 min say meh", initial_time)
        self.assertEqual(p.get_time_string().lower(), "2019-08-21 at 00:18 pm")
        
        p = SlashCommandParser("monday say meh", initial_time)
        self.assertEqual(p.get_time_string().lower(), "2019-08-26 at 00:18 am")
        
        p = SlashCommandParser("8 p.m. say meh", initial_time)
        self.assertEqual(p.get_time_string().lower(), "2019-08-19 at 08:00 pm")
        
        p = SlashCommandParser("september 1 say meh")
        self.assertEqual(p.get_time_string().lower(), "2019-09-01 at 00:00 am")
        
        p = SlashCommandParser("2019-09-01 14:30 say Take break")
        self.assertEqual(p.get_time_string().lower(), "2019-09-01 at 02:30 pm")
